...
@Injectable()
export class CalendarEventDetailStore extends ComponentStore<CalendarEventDetailState> {
  constructor(
  ...
  readonly deleteCalendarEventEffect = this.effect<CalendarEvent>((calendarEvent$) =>
    calendarEvent$.pipe(
      switchMap((calendarEvent) =>
        this.data
          ...
          .pipe(
            tapResponse(
              (res) => this.router.navigate(['/calendar-events']),
              (errors: any) =>
                this.patchState({
                  loading: false,
                  errors: errors.graphQLErrors ? errors.graphQLErrors : errors,
