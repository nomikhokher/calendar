...
@Component({
    ...
    <ng-container *ngIf="vm$ | async as vm">
      ...
      <ng-container *ngIf="!vm.loading">
        <ng-container *ngIf="!vm.items?.length">
          <div class="flex py-20 justify-center align-center">No Calendar Weekday found...</div>
        </ng-container>
                    ...
                    <div class="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
                      <div class="truncate">
                        <div class="flex text-sm">
                          <p class="font-medium text-indigo-600 truncate">{{ item?.abbr }}</p>
                          <p class="ml-3 font-medium text-indigo-600 truncate">{{ item?.label }}</p>
                        </div>
  ...
  `,
  providers: [AdminCalendarWeekdayListStore],
})
export class AdminCalendarWeekdayListComponent implements OnInit {
  readonly vm$ = this.store.vm$
  constructor(private readonly store: AdminCalendarWeekdayListStore) {}
